...
export const CommanderContainer = ({theme}: CommanderProps) => {
    ...
    const onImgNotFound = (evt: React.SyntheticEvent<HTMLImageElement>) => 
        evt.currentTarget.parentElement?.classList.add("defaultImg")
            ...
            <td key={1}>
                ...
                :
                    <span>
                        ...
                        <img className="image" src={`http://localhost:3333/geticon?ext=.${ext}`} onError={onImgNotFound} ></img>
                    </span>
                }
                <span>{tableItem.name}</span>
            </td>,
